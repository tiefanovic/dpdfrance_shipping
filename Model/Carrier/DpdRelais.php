...
class DpdRelais extends \Magento\Shipping\Model\Carrier\AbstractCarrier implements
    \Magento\Shipping\Model\Carrier\CarrierInterface
{
    ...
    {
        return ['dpdfrrelais' => $this->_scopeConfig->getValue('carriers/dpdfrrelais/name')];
    }
    ...
    public function collectRates(RateRequest $request)
    {
       ...
       /* die(var_dump($this->getConfigFlag('active')));
		...
		$method->setCarrier('dpdfrrelais');
		$method->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrrelais/title'));
 
		$method->setMethod('dpdfrrelais');
		$method->setMethodTitle($this->_scopeConfig->getValue('carriers/dpdfrrelais/name'));
 
		if($this->_scopeConfig->getValue('carriers/dpdfrrelais/rate_type') == 'table')
		{
    ...
    protected function _process(&$process) {
        ...
        if (!$value_found && $this->_scopeConfig->getValue('carriers/dpdfrrelais/showerror'))
            $this->_setError($process, $this->_scopeConfig->getValue('carriers/dpdfrrelais/specificerrmsg'));
    }
    protected function _getConfig() {
        if (!isset($this->_expenseConfig)) {
            $this->_owebiaCore = new Owebia($this->_scopeConfig->getValue('carriers/dpdfrrelais/expense'));
    ...
    protected function _setError(&$process, $message) {
        ...
        $error->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrrelais/title'));
    ...
    protected function _appendMethod($process, $row, $fees) {
        ...
        $method = $this->methodFactory->create()
            ...
            ->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrrelais/title'))
            ->setMethod($row['*code'])
            ->setMethodTitle($this->_scopeConfig->getValue('carriers/dpdfrrelais/name') . ' ' . $this->_getMethodText($process,$row,'label'))
            ->setPrice($fees)
            ->setCost($fees);
