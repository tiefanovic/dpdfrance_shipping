...
class DpdClassic extends \Magento\Shipping\Model\Carrier\AbstractCarrier implements
    \Magento\Shipping\Model\Carrier\CarrierInterface
{
	...
	/**
	 * @var \DPDFrance\Shipping\Helper\TrackingPopupFactory
	 */
    ...
    /**
     * @var \DPDFrance\Shipping\Model\ResourceModel\TablerateFactory
     */
    ...
    {
        return ['dpdfrclassic' => $this->_scopeConfig->getValue('carriers/dpdfrclassic/name')];
    }
    ...
    public function getTrackingInfo($trackingNumber)
    {
        ...
        $cargo = $this->_scopeConfig->getValue('carriers/dpdfrclassic/cargo');
        $longueurcargo = strlen($cargo);
        $cargoutilise = substr($trackingNumber, -(int) $longueurcargo);
        $result = $this->getResult();
        if($cargoutilise == $cargo){
            $tracking = $this->trackStatusFactory->create();
            $tracking->setCarrier('dpdfrclassic');
            $tracking->setCarrierTitle('DPD Relais');
    ...
    public function collectRates(RateRequest $request)
    {
       ...
       /* die(var_dump($this->getConfigFlag('active')));
		...
		$method->setCarrier('dpdfrclassic');
		$method->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrclassic/title'));
 
		$method->setMethod('dpdfrclassic');
		$method->setMethodTitle($this->_scopeConfig->getValue('carriers/dpdfrclassic/name'));
 
		if($this->_scopeConfig->getValue('carriers/dpdfrclassic/rate_type') == 'table')
		{
    ...
    protected function _process(&$process) {
        ...
        if (!$value_found && $this->_scopeConfig->getValue('carriers/dpdfrclassic/showerror'))
            $this->_setError($process, $this->_scopeConfig->getValue('carriers/dpdfrclassic/specificerrmsg'));
    }
    protected function _getConfig() {
        if (!isset($this->_expenseConfig)) {
            $this->_owebiaCore = new Owebia($this->_scopeConfig->getValue('carriers/dpdfrclassic/expense'));
    ...
    protected function _setError(&$process, $message) {
        ...
        $error->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrclassic/title'));
    ...
    protected function _appendMethod($process, $row, $fees) {
        ...
        $method = $this->methodFactory->create()
            ...
            ->setCarrierTitle($this->_scopeConfig->getValue('carriers/dpdfrclassic/title'))
            ->setMethod($row['*code'])
            ->setMethodTitle($this->_scopeConfig->getValue('carriers/dpdfrclassic/name') . ' ' . $this->_getMethodText($process,$row,'label'))
            ->setPrice($fees)
            ->setCost($fees);
